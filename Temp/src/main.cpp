...
void initTFT() {
  ...
  tft.drawString("启动中...", tft.width() / 2, tft.height() / 2);
  delay(1000);
}
...
void drawStaticElements() {
  ...
  tft.drawString("ESP32 Temp:", DATA_X + 10, DATA_Y + 13 * LINE_HEIGHT);
}
...
void updatePCData() {
  ...
  tft.setTextColor(VALUE_COLOR, BG_COLOR);
  tft.drawString(String(esp32c3_temp, 1) + "°C", DATA_X + 80, DATA_Y + 13 * LINE_HEIGHT);
