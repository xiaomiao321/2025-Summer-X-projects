...
struct lsm6dsm_data *LSM6DSM;  extern struct lsm6dsm_data *LSM6DSM_sysVar(void);
struct lis3mdl_data *LIS3MDL;  extern struct lis3mdl_data *LIS3MDL_sysVar(void);
...
void systemVariableInit(void) {
	...
	LSM6DSM = LSM6DSM_sysVar();  // 6轴传感器数据
	LIS3MDL = LIS3MDL_sysVar();  // 磁力计数据
}
