...
void MqttProtocol::SendMcpMessage(const std::string& message) {
    std::string topic = "xiaozhi/mcp/up/" + GetClientId();
    esp_mqtt_client_publish(client_, topic.c_str(), message.c_str(), 0, 1, 0);
}

void MqttProtocol::Publish(const std::string& topic, const std::string& payload) {
    if (!client_) {
