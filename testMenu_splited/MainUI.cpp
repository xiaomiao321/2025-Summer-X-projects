...
// const int icon_spacing = 56;       // 图标间距，调整以适应屏幕宽度
// // 菜单项结构
// struct MenuItem {
...
// static int16_t menu_display = 10;  // 菜单文字初始y坐标
// static int16_t menu_display_trg = 110; // 文字目标y坐标，移至屏幕底部
...
//     menuSprite.fillTriangle(triangle_x, 90, triangle_x - 12, 70, triangle_x + 12, 70, TFT_WHITE);

//     // 图标（Y=20）
//     for (int i = 0; i < MENU_ITEM_COUNT; i++) {
//         int16_t x = offset + (i * icon_spacing);
//         if (x >= -icon_size && x < 128) {
//             menuSprite.pushImage(x, 40, icon_size, icon_size, menuItems[i].image);
//         }
//     }
...
//     menuSprite.drawString("Select", 10, 110); // 底部提示
...
//     int16_t start_y = entering ? 10 : 110;
//     int16_t target_y = entering ? 110 : 10;
//     int16_t animated_text_y = start_y;

//     for (uint8_t step = ANIMATION_STEPS; step > 0; step--) {
//         tft.fillRect(0, 0, 128, 128, TFT_BLACK); // 清全屏

//         // 绘制图标（y=40）
//         for (int i = 0; i < MENU_ITEM_COUNT; i++) {
//             int16_t x = display + (i * icon_spacing);
//             if (x >= -icon_size && x < 128) {
//                 tft.pushImage(x, 40, icon_size, icon_size, menuItems[i].image);
//             }
//         }

//         // 三角形（y=70-90）
//         int16_t triangle_x = display + (picture_flag * icon_spacing) + icon_size / 2;
//         tft.fillTriangle(triangle_x, 90, triangle_x - 12, 70, triangle_x + 12, 70, TFT_WHITE);

//         // 动画文字
//         tft.setTextColor(TFT_WHITE, TFT_BLACK);
//         tft.setTextSize(1);
//         tft.setTextDatum(TL_DATUM);
//         tft.drawString("MENU:", 10, animated_text_y);
//         tft.drawString(title, 50, animated_text_y);
...
//         int16_t target_display = 64 - (picture_flag * icon_spacing);
